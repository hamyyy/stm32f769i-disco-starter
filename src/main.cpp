...
static void LCD_Show_Feature(uint8_t feature)
{
  ...
  switch (feature) {
    ...
    case 2:
      ...
      if (bmpImg != nullptr) {
        BSP_LCD_DrawBitmap(Xsize / 2 - 40, 100, bmpImg);
        HAL_Delay(500);
      }
...
{
  {
    ...
    for (i = 0; i < height; i++) {
      for (j = 0; j < width; j++) {
        image[i * width * BYTES_PER_PIXEL + j * BYTES_PER_PIXEL + 2] = (unsigned char)(i * (uint32_t)255 / (uint32_t)height);  // red
        image[i * width * BYTES_PER_PIXEL + j * BYTES_PER_PIXEL + 1] = (unsigned char)(j * (uint32_t)255 / (uint32_t)width);   // green
        image[i * width * BYTES_PER_PIXEL + j * BYTES_PER_PIXEL + 0] = 0;                                  // (unsigned char)((i + j) * 255 / (height + width));  // blue
      }
    }

    bmpImg = generateBitmapImage((unsigned char *)image, height, width, imageFileName);
